...
def project_recurring_events(
    ...
    for event in master_events_response.items:
        ...
        if event.start.dateTime:
            try:
                dtstart_obj = event.start.dateTime
                if event.end and event.end.dateTime:
                    dtend_obj = event.end.dateTime
        ...
        try:
            ...
            main_rule = cast(rrule.rrule, rrule.rrulestr(rrule_str, dtstart=dtstart_obj))
            ruleset.rrule(main_rule) # Add the parsed rule to the set

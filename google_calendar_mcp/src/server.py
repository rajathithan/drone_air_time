...
# Mount FastMCP HTTP transport for agent clients (SSE by default).
mcp_mount_path = os.getenv("MCP_HTTP_MOUNT_PATH", "/mcp")
mcp_http_transport = os.getenv("MCP_HTTP_TRANSPORT", "sse").lower()
try:
    mcp_server = create_mcp_server()
    if mcp_http_transport == "sse":
        # When mounted under FastAPI, let Starlette root_path provide the mount prefix.
        # Passing mount_path here duplicates it in the advertised /messages endpoint.
        app.mount(mcp_mount_path, mcp_server.sse_app())
        logger.info(f"Mounted MCP SSE app at '{mcp_mount_path}' (SSE at '{mcp_mount_path}/sse').")
    elif mcp_http_transport in {"streamable-http", "streamable_http"}:
        app.mount(mcp_mount_path, mcp_server.streamable_http_app())
        logger.info(f"Mounted MCP Streamable HTTP app at '{mcp_mount_path}'.")
    elif mcp_http_transport in {"none", "disabled", "off"}:
        logger.info("MCP HTTP transport disabled via MCP_HTTP_TRANSPORT.")
    else:
        logger.warning(
            f"Unsupported MCP_HTTP_TRANSPORT='{mcp_http_transport}'. Expected 'sse', 'streamable-http', or 'none'. "
            "Falling back to SSE."
        )
        app.mount(mcp_mount_path, mcp_server.sse_app())
except Exception as e:
    logger.error(f"Failed to initialize MCP HTTP transport: {e}", exc_info=True)
...
global_credentials: Optional[OAuthCredentials] = None
...
def startup_event():
    ...
    try:
        global_credentials = cast(Optional[OAuthCredentials], get_credentials())
...
def get_current_credentials() -> OAuthCredentials:
...
def list_calendars_endpoint(
    ...
    creds: OAuthCredentials = Depends(get_current_credentials)
):
...
def query_free_busy_endpoint(
    ...
    return FreeBusyResponse(
        timeMin=request.time_min, # Echo request params as per Google API response structure

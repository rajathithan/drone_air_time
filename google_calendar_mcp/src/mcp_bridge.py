...
REQUEST_TIMEOUT_SECONDS = float(os.getenv("MCP_BRIDGE_HTTP_TIMEOUT_SECONDS", "60"))
...
def create_mcp_server():
    ...
    async def _request_in_thread(method: str, url: str, **kwargs) -> requests.Response:
    ...
    async def list_calendars(min_access_role: Optional[str] = None) -> str:
        ...
        try:
            ...
            response = await _request_in_thread("GET", f"{BASE_URL}/calendars", params=params)
    ...
    async def find_events(calendar_id: str, time_min: Optional[str] = None, 
        ...
        try:
            ...
            response = await _request_in_thread("GET", f"{BASE_URL}/calendars/{calendar_id}/events", params=params)
            if response.status_code != 200:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def create_event(calendar_id: str, summary: str, start_time: str, 
        ...
        try:
            ...
            response = await _request_in_thread(
                "POST",
    ...
    async def delete_event(calendar_id: str, event_id: str) -> str:
        ...
        """
        try:
            response = await _request_in_thread("DELETE", f"{BASE_URL}/calendars/{calendar_id}/events/{event_id}")
    ...
    async def check_attendee_status(event_id: str, calendar_id: str = "primary", 
        ...
        try:
            ...
            response = await _request_in_thread("POST", f"{BASE_URL}/events/check_attendee_status", json=data)
            if response.status_code != 200:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def query_free_busy(calendar_ids: List[str], time_min: str, time_max: str) -> str:
        ...
        try:
            ...
            response = await _request_in_thread("POST", f"{BASE_URL}/freeBusy", json=data)
            if response.status_code != 200:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def schedule_mutual(attendee_calendar_ids: List[str], time_min: str, 
        ...
        try:
            ...
            response = await _request_in_thread("POST", f"{BASE_URL}/schedule_mutual", json=data)
            if response.status_code != 201:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def analyze_busyness(time_min: str, time_max: str, calendar_id: str = "primary") -> str:
        ...
        try:
            ...
            response = await _request_in_thread("POST", f"{BASE_URL}/analyze_busyness", json=data)
            if response.status_code != 200:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def create_calendar(summary: str) -> str:
        ...
        try:
            data = {"summary": summary}
            response = await _request_in_thread("POST", f"{BASE_URL}/calendars", json=data)
            if response.status_code != 201:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})

...
if __name__ == "__main__":
    ...
    # Start MCP stdio server only when explicitly requested.
    if mcp_transport == "stdio":
        logger.info("MCP_TRANSPORT=stdio: Starting MCP server thread")
        mcp_thread = threading.Thread(target=run_mcp_server, daemon=True)
        mcp_thread.start()
        logger.info("MCP server thread launched")
    else:
        logger.info(f"Running in HTTP mode (MCP_TRANSPORT={mcp_transport})")
        # Ensure console handler is present in HTTP mode
        root_logger = logging.getLogger()
